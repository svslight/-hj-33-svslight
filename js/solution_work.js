...
// Копирование ссылки и переход из режима Поделиться  в Комментирование
...
//  commentsMarker.parentElement.style.display = 'none';
  ...
  if (showedForm) {       
    console.log('2 Есть showedForm - Удалить пустую форму или свернуть форму с сообщениями, маркер оставить', showedForm);
      
    hideForm(showedForm); 
      
    console.log('3 showedForm - isShowedForm(showedForm)', isShowedForm(showedForm));     
  } 
...
//  (isShowedForm(showedForm)) ? hideForm(showedForm) : showForm(showedForm);  
  ...
  btnCommentsClose.addEventListener('click', event => { 
    ...
    if (commentMessage) {
      ...
      console.log('+++ Кнопка-Есть комментарий - свернуть форму', commentMessage); 
      commentsBody.style.display = 'none';     // свернуть форму
      //      checkboxMarker.checked = false;  
        
    } else {    
        
      console.log('??? Кнопка-Нет комментарий - удалить маркера и свернуть форму', commentMessage);        
      formComment.removeChild(commentsMarker);  // уд. маркера
      commentsBody.style.display = 'none';     // свернуть свернуть
      //      checkboxMarker.checked = false;      
    }
...
function addMessageComment(message, form) {
  ...
  console.log('Добавление комментария в форму - parentLoaderDiv', parentLoaderDiv);
